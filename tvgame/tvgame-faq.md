...
一般来说，要集成外设和支付。这是两个毫不相关的任务。外设不需要账号联调，所以，建议先外设后支付。单机免费游戏当然就不用集成支付了。在此向单机免费游戏的开发者们致以最崇高的敬意！
...
只有在接入支付时才需要注册开发者（身份证明和企业支付宝），创建应用获得 `App Key`, `App Secret` 和 `Baodian Secret` ，前两者都是[淘宝开放平台](http://open.taobao.com)上可以直接获得的，第三个目前（ 2015 年 3 月）还没有外部可获得通道，需要将`开发者帐号`、`应用名` 和 `App Key` 发给相关人员操作生成。估计一两个月后可能有点改观吧。
...
从产品功能上看，YunOS TV 支付系统是“经典款”，而宝点支付系统则是持续优化的“时尚款”，`YunOS TV 2.3.0` 及以上系统才支持。
